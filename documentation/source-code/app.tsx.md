...
**Role:** Root React component. Orchestrates data loading, owns all top-level state, renders the page header with the Cards/List toggle, and renders one `CategorySection` per category group.
...
|---------------|------|---------|
| `categories` | `CategoryGroup[]` | Grouped dashboard data returned by `DashboardService` |
| `orderMap` | `OrderMap` | Per-category ordered sys_id arrays from `PreferenceService` |
| `loading` | `boolean` | Shows spinner while data is in flight |
| `error` | `string \| null` | Shows error banner if either fetch fails |
| `viewMode` | `'grid' \| 'list'` | Controls which layout `CategorySection` renders |

---

## Data loading

On mount, `useEffect` fires **one** async block that runs both service calls in parallel:
...
Both services are created with `useMemo(() => new Service(), [])` so they are constructed once and stable across renders.
...
Called by `CategorySection` after each drag-and-drop with the new ordered sys_id array for that category. Updates local state immediately (optimistic) and persists to `sys_user_preference` (debounced inside `CategorySection`).
